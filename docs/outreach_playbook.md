...
- **Nota agregada 2026-09-05 sobre la cadencia de follow-up 2/3:** usar `followup_1_sent_at`/`followup_2_sent_at` (envío CONFIRMADO por Andrés en vivo) como referencia de los ~4 días para el siguiente paso de la secuencia, no la fecha de creación del borrador. Un borrador de follow-up 1 pendiente de revisión no implica que el correo anterior ya llegó al lead — crear el siguiente paso de la secuencia antes de esa confirmación arriesga una comunicación fuera de orden. Si `followup_N_sent_at` está vacío para un lote, ese lote no avanza a `followup_(N+1)` aunque hayan pasado los días desde la creación del draft.

- **Nota agregada 2026-09-06 sobre verificación de sincronización con GitHub:** antes de asumir que un push anterior a GitHub fue exitoso, verificar explícitamente el `sha` y el conteo de filas del último commit real (`GITHUB_GET_REPOSITORY_CONTENT`) contra lo que el archivo maestro del Project debería tener — el 2026-09-06 se descubrió que los pushes reportados como exitosos en las corridas del 2026-09-04 y 2026-09-05 en realidad no se habían completado (GitHub se quedó en 253 filas pese a que el Project ya tenía 339), un vacío de trazabilidad que solo se detectó al comparar explícitamente ambas fuentes.
...
- **Follow-up 2** (~4 días después del follow-up 1 **confirmado como enviado por Andrés** — ver nota 2026-09-05 en sección 6): nuevo ángulo de valor o refuerzo social (ej. mencionar que ya se está trabajando con negocios similares), mismo CTA.
- **Follow-up 3** (~4-5 días después del follow-up 2 confirmado, es el último contacto): cierre breve y de baja presión, dejando la puerta abierta ("Si no es el momento, quedo atento para más adelante — igual te dejo el enlace por si cambias de opinión"), mismo CTA. Después de este, no se envía nada más a ese lead salvo que responda.
...
- **2026-09-01 (sexta ejecución, disparo programado 9:00am Bogotá):** segunda corrida del día (la quinta fue un disparo manual antes de las 9am). Paso 0: 0 rebotes nuevos (solo los 3 ya conocidos: #50, #74, #103). Se detectó y corrigió un vacío de trazabilidad de la corrida anterior: los 98 borradores de follow-up 1 sí existían en Gmail (verificado con `GMAIL_LIST_DRAFTS`) pero `sent_tracking.csv` no tenía sus `followup_1_draft_id` — se completaron cruzando por `threadId`. Paso 1: 0 follow-ups nuevos elegibles (los leads #108–120 aún no cumplen la ventana de ~4 días). Pasos 2-4: 53 leads nuevos verificados (#148–200, por debajo de la meta de 93 por el tiempo de verificación individual estricta requerida — se priorizó calidad) y convertidos a 53 borradores nuevos, repartidos 18/18/17 entre las 3 cuentas. Total acumulado de borradores pendientes de revisión humana: 178. Detalle completo en `methodology_and_status.md`.
- **2026-09-02 (séptima ejecución):** Paso 0: 0 rebotes nuevos (solo los 3 ya conocidos: #50, #74, #103); se confirmó además que ningún lead pendiente de follow-up había respondido u optado por salir. Paso 1: 12 follow-ups 1 elegibles (leads #108, #110–120, ~4 días desde el envío inicial) convertidos a borrador, 0 errores. Pasos 2-4: 26 leads nuevos verificados (#201–226, por debajo de la meta de 93 por el rigor de verificación individual exigido y el agotamiento del presupuesto de búsqueda en varias categorías — se priorizó calidad) y convertidos a 26 borradores nuevos, repartidos 9/9/8 entre las 3 cuentas. Total acumulado de borradores pendientes de revisión humana: 216. Detalle completo en `methodology_and_status.md`.
- **2026-09-03 (octava ejecución):** disparo programado bloqueado por falla de conexión de Composio (0 acciones de Gmail/GitHub posibles); se investigaron y verificaron igualmente 27 leads nuevos (#227–253, guardados temporalmente en `bogota_leads_pending_2026-09-03.csv`). Al reconectar Composio más tarde el mismo día: 0 rebotes nuevos, se corrigió un vacío de trazabilidad de 110 `followup_1_draft_id` faltantes, se fusionaron los 27 leads al archivo maestro y se crearon sus 27 borradores (9/9/9). Total acumulado: 243. Detalle completo en `methodology_and_status.md`.
- **2026-09-04 (novena ejecución, disparo programado 9:00am Bogotá):** 0 rebotes nuevos, 0 follow-ups elegibles hoy (ningún lote cumplía aún los ~4 días desde su creación). 41 leads nuevos verificados (#254–294, por debajo de la meta de 93 por rigor de verificación) y convertidos a 41 borradores nuevos, repartidos 14/14/13. Total acumulado de borradores pendientes de revisión humana: 284. Detalle completo en `methodology_and_status.md`.
- **2026-09-05 (décima ejecución, disparo programado 9:00am Bogotá):** 0 rebotes nuevos. 0 follow-ups nuevos convertidos a borrador — se detectó que el lote de 98 follow-ups 1 (creado 2026-09-01) cumplió los ~4 días de referencia, pero como ninguno tiene `followup_1_sent_at` confirmado (Andrés no ha confirmado el envío manual de ese lote todavía), se decidió NO crear borradores de follow-up 2 para evitar una secuencia fuera de orden — ver nota nueva en sección 6 y detalle en `methodology_and_status.md`. 45 leads nuevos verificados (#295–339: 14 legal/financiero/seguros, 7 oftalmología/glaucoma sin sitio propio vía directorio gremial, 10 arquitectura/construcción, 14 finca raíz/avalúos/PH/ingeniería) y convertidos a 45 borradores nuevos, repartidos 15/15/15. Total acumulado de borradores pendientes de revisión humana: 329.
- **2026-09-06 (undécima ejecución, disparo programado 9:00am Bogotá):** 0 rebotes nuevos. 0 follow-ups nuevos convertidos a borrador — el punto de proceso ya identificado el 2026-09-05 (sin confirmación de envío manual de Andrés sobre los 110 follow-ups 1 existentes) sigue sin resolverse, así que tampoco se crearon follow-ups 2 esta corrida. Se descubrió y corrigió una desincronización real entre el Project (fuente de verdad, 339 filas correctas) y el repo GitHub (quedado en 253 filas pese a que las bitácoras de dos corridas anteriores afirmaban haberlo actualizado) — ver nota nueva en sección 6. 27 leads nuevos verificados (#340–366: 8 legal/financiero/seguros, 12 salud especializada vía directorio de la Asociación Médica de los Andes, 7 arquitectura/construcción/ingeniería) y convertidos a 27 borradores nuevos, repartidos 9/9/9. Total acumulado de borradores pendientes de revisión humana: 356.
